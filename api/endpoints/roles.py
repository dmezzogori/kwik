...
def read_roles(db: Session = kwik.db, skip: int = 0, limit: int = 100,) -> Any:
    """
    Retrieve roles.
    """
    # TODO total count
    _, roles = crud.role.get_multi(db=db, skip=skip, limit=limit)
    return roles
...
def associate_user_to_role(*, db: Session = kwik.db, user_role_in: schemas.UserRoleCreate) -> Any:
    ...
    if not user:
        raise HTTPException(
            status_code=412, detail="The specified user does not exists in the system.",
        )
...
def update_role(*, db: Session = kwik.db, role_id: int, role_in: schemas.RoleUpdate,) -> Any:
    ...
    if not role:
        raise HTTPException(
            status_code=404, detail="The role with this id does not exist in the system",
        )
