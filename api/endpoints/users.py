...
@router.get("/", response_model=List[schemas.User], dependencies=[kwik.has_permission('user_management')])
def read_users(
    db: Session = kwik.db,
    skip: int = 0,
    limit: int = 100,
...
@router.post("/", response_model=schemas.User, dependencies=[kwik.has_permission('user_management')])
def create_user(
    *,
    db: Session = kwik.db,
