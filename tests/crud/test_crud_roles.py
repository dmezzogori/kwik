...
class TestRoleCRUD:
    ...
    def test_create_role(self) -> None:
        ...
        created_role = crud_roles.create(obj_in=role_data)
    ...
    def test_get_role_by_id(self) -> None:
        ...
        # Get the role by ID
        retrieved_role = crud_roles.get(id=role.id)

        assert retrieved_role is not None
        assert retrieved_role.id == role.id
        assert retrieved_role.name == "Get Test Role"

    def test_get_role_by_nonexistent_id_returns_none(self) -> None:
        """Test getting a role by non-existent ID returns None."""
        retrieved_role = crud_roles.get(id=99999)
    ...
    def test_update_role(self) -> None:
        ...
        updated_role = crud_roles.update(db_obj=role, obj_in=update_data)
    ...
    def test_get_if_exist_with_existing_role(self) -> None:
        """Test get_if_exist with an existing role."""
        # Create a test role
        role = create_test_role()

        # Get the role using get_if_exist
        retrieved_role = crud_roles.get_if_exist(id=role.id)

        assert retrieved_role.id == role.id

    def test_get_if_exist_with_nonexistent_role_raises_error(self) -> None:
        """Test get_if_exist with non-existent role raises NotFound."""
        with pytest.raises(EntityNotFoundError):
            crud_roles.get_if_exist(id=99999)

    def test_get_multi_roles(self) -> None:
        ...
        # Get multiple roles
        count, retrieved_roles = crud_roles.get_multi(skip=0, limit=page_limit)

        assert count == total_roles  # Total count
        assert len(retrieved_roles) == page_limit  # Limited to 3

        # Test pagination
        count, second_page = crud_roles.get_multi(skip=page_limit, limit=page_limit)
    ...
    def test_get_multi_with_filters(self) -> None:
        ...
        create_test_role(name="Inactive Role", is_active=False)

        # Filter by is_active
        count, active_roles = crud_roles.get_multi(is_active=True)
        expected_active_roles = 1  # Active Role
        assert count == expected_active_roles

        count, inactive_roles = crud_roles.get_multi(is_active=False)
        assert count == 1
    ...
    def test_delete_role(self) -> None:
        ...
        # Create a test role
        role = create_test_role(name="To Delete")
        role_id = role.id

        # Delete the role (hard delete)
        deleted_role = crud_roles.delete(id=role_id)

        assert deleted_role.id == role_id

        # Verify role is completely removed
        retrieved_role = crud_roles.get(id=role_id)
