...
from tests.utils import create_test_permission, create_test_user
...
class TestPermissionCRUD:
    ...
    def test_create_permission(self, db_session: Session) -> None:
        ...
        created_permission = crud.crud_permissions.create(obj_in=permission_data)
    ...
    def test_create_permission_duplicate_name_allowed(self, db_session: Session) -> None:
        ...
        perm1 = crud.crud_permissions.create(obj_in=permission_data)
        ...
        perm2 = crud.crud_permissions.create(obj_in=permission_data2)
    ...
    def test_update_permission(self, db_session: Session) -> None:
        ...
        updated_permission = crud.crud_permissions.update(db_obj=permission, obj_in=update_data)
    ...
    def test_get_if_exist_with_nonexistent_permission_raises_error(self, db_session: Session) -> None:
        ...
        with pytest.raises(EntityNotFoundError):
            crud.crud_permissions.get_if_exist(id=99999)
    ...
    def test_get_multi_permissions(self, db_session: Session) -> None:
        ...
        count, retrieved_permissions = crud.crud_permissions.get_multi(skip=0, limit=page_limit)

        assert count == total_permissions  # Total count
        assert len(retrieved_permissions) == page_limit  # Limited to 3

        # Test pagination
        count, second_page = crud.crud_permissions.get_multi(skip=page_limit, limit=page_limit)
    ...
    def test_basic_delete_permission_direct(self, db_session: Session) -> None:
        ...
        retrieved_permission = crud.crud_permissions.get(id=permission_id)
        assert retrieved_permission is None
